...
export type Options = {
  ...
  offsetSize?: boolean
  offsetCoords?: boolean
  onChange?: (bounds: RectReadOnly) => void
}
...
export function useMeasure(
  ...
  const [forceRefresh, resizeChange, scrollChange] = useMemo(() => {
    const callback = () => {
      ...
      if (state.current.element instanceof HTMLElement && offsetCoords) {
        size.offsetTop = state.current.element.offsetTop
        size.offsetLeft = state.current.element.offsetLeft
      }
